...
def chen(home, season, folder_path, how="max"):
    ...
    for day in days:
        ...
        if len(night_df):
            ...
            if how is "mean":
                avg_power_threshold = df.mean()
                avg_range_threshold = df.mean()
                avg_sd_threshold = df.mean()
            else:
                avg_power_threshold = df.max()
                avg_range_threshold = df.max()
                avg_sd_threshold = df.max()
...
def classify_train_test_same_home(folder_path):
    ...
    for season in ["summer", "winter"]:
        ...
        #for home in  [4]:
            ...
            dfs = []
            for train_home in train_homes:
                home_path = season_path+"/"+str(train_home)+".csv"
                df = pd.read_csv(home_path, index_col=0)
                len_df = len(df)
                df = df.head(len_df/2)
                df.index = pd.to_datetime(df.index)
            ...
            out[season][home]["Chen"] = chen(home, season,
                                                                   os.path.expanduser("~/Dropbox/niomtk_data/eco/1min/"))
            out[season][home]["Chen-mean"] = chen(home, season,
                                                                   os.path.expanduser("~/Dropbox/niomtk_data/eco/1min/"), how="mean")
...
def plot_train_single_averaged(accuracy_dict=None):
    ...
    summer_dict = {}
    winter_dict = {}
    for metric in ['tp','tn','fp','fn','accuracy']:
        if len(metric)==2:
            metric_name = metric.upper()
        else:
            metric_name = metric.capitalize()
        summer_dict[metric_name] = pd.DataFrame(res['summer'][metric]).T.mean()
        winter_dict[metric_name] = pd.DataFrame(res['winter'][metric]).T.mean()

        summer_df = pd.DataFrame(summer_dict).T
        summer_df = summer_df[['Chen','Our approach',  'KNN', 'SVM', 'RF']]
        #summer_df.columns = ['Chen', 'Our\napproach','KNN','SVM','RF']

        winter_df = pd.DataFrame(winter_dict).T
        winter_df = winter_df[['Chen', 'Our approach',  'KNN', 'SVM', 'RF']]
        #winter_df.columns = ['Chen', 'Our\napproach','KNN','SVM','RF']

    latexify(columns=2, fig_height=3.0)

    fig, ax = plt.subplots(nrows=2, sharey=True, sharex=True)
    print len(ax)
    summer_df.plot(kind='bar', rot=0, ax=ax[0], title="Summer")
    winter_df.plot(kind='bar', rot=0, ax=ax[1], title="Winter",legend=False)
    plt.ylim((0, 1.1))
    #plt.figlegend((line, summer_df.columns),ncol=5, loc='upper center', bbox_to_anchor=(0.5, 1.2))
    ...
    #plt.ylim((0, 1.1))
    ax[0].legend(ncol=5, loc='upper center', bbox_to_anchor=(0.5, 1.4))
    #format_axes(ax)
    #plt.tight_layout()
    plt.tight_layout()
    plt.savefig(os.path.expanduser("~/git/NIOMTK/figures/eco.pdf"), bbox_inches="tight")
    #import matplotlib.pyplot as plt
    #latexify(columns=1)
    plot_tp_fp_fn_both(summer_df, winter_df)
...
def plot_tp_fp_fn_both(summer_df, winter_df):
    latexify(columns=2, fig_height=2.)
    fig, ax = plt.subplots(ncols=2, sharey=True)
    for i, results_df in enumerate([summer_df, winter_df]):
        results_df.loc['TP+FP'] = results_df.loc['TP'] + results_df.loc['FP']
        y = results_df.ix['TP+FP']
        x = results_df.ix['FN']
        results_df.loc['TP+FP'] = results_df.loc['TP'] + results_df.loc['FP']
        for j,(x_p, y_p) in enumerate(zip(x, y)):
            ax[i].plot(x_p, y_p, marker='o', linestyle='', label=x.index[j],ms=6, alpha=0.7)
        ax[i].set_xlabel("Miss time (FN)")
        format_axes(ax[i])
    ...
    ax[0].legend(ncol=5, loc='upper center',scatterpoints = 1,numpoints=1, bbox_to_anchor=(1, 1.4))
    #ax[1].legend(loc='center left', bbox_to_anchor=(1, 0.5))


    fig.tight_layout()
    plt.savefig(os.path.expanduser("~/git/NIOMTK/figures/eco_scatter.pdf"),bbox_inches="tight")
...
def plot_tp_fp_fn(results_df, plot_name):
    ...
    for i,(x_p, y_p) in enumerate(zip(x, y)):
        plt.plot(x_p, y_p, marker='o', linestyle='', label=x.index[i],ms=6, alpha=0.7)
